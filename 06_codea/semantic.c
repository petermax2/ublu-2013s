...
/* DEBUG function: print parameter list to stdout */
void printSym(struct symT *first) 
{
	struct symT *p = first;
	fprintf(stdout, "----- BEGIN OF SYMBOL TABLE -----\n");
	while(p != NULL) 
	{
		fprintf(stdout, "var %s : type %d \n", p->name, p->type);
		p = p->next;
	}
	fprintf(stdout, "----- END OF SYMBOL TABLE -----\n");
}
